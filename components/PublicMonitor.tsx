...
import { ArrowLeft, Megaphone, Loader2, Clock, MapPin, Truck, FileText } from 'lucide-react';
...
const PublicMonitor: React.FC<Props> = ({ onBack }) => {
  ...
  // Waiting list are those verified (Security approved) but not yet Called
  const waitingDrivers = drivers
      .filter(d => d.status === QueueStatus.VERIFIED)
      .sort((a,b) => (a.verifiedTime || 0) - (b.verifiedTime || 0));

  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 overflow-hidden font-sans relative">
        ...
        {/* Back Button for Mobile/Public Users */}
        {onBack && (
            <button 
                onClick={onBack}
                className="absolute top-4 left-4 md:top-8 md:left-8 z-50 p-3 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-sm transition-all border border-white/10"
                        ...
                        return (
                            <div 
                                ...
                                className={`relative p-6 rounded-2xl border-l-8 shadow-2xl transition-all duration-500 overflow-hidden group
                                    ${isCalled 
                                        ? 'bg-gradient-to-r from-amber-900/40 to-slate-900 border-amber-500 shadow-amber-900/20' 
                            ...
                            >
                                ...
                                {isCalled && (
                                    <div className="absolute inset-0 bg-amber-500/5 animate-pulse z-0 pointer-events-none"></div>
                                )}
                                ...
                                <div className="relative z-10 flex justify-between items-center">
                                    <div className="flex-1 min-w-0 pr-4">
                                        ...
                                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider mb-2
                                            ${isCalled ? 'bg-amber-500 text-slate-900 animate-pulse' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'}`}>
                                            {isCalled ? (
                                    ...
                                    <div className="text-right shrink-0">
                                        ...
                                        <div className={`text-7xl font-black tracking-tighter drop-shadow-lg
                                            ${d.gate === 'GATE_2' ? 'text-blue-400' : 'text-purple-400'}`}>
                    ...
                    {waitingDrivers.length === 0 ? (
                        ...
                        >
                            ...
                            <div className="col-span-3 text-right">
                                <span className={`text-2xl font-black font-mono ${d.gate === 'GATE_2' ? 'text-blue-400' : 'text-purple-400'}`}>
