...
import { 
  ...
  AlertTriangle, Copy, X, Truck, Lock, UserCheck, Key, CheckCircle, Warehouse, Download, Upload, Edit, Server, Layout, Heart, User, Eye, EyeOff, ShieldCheck, LayoutDashboard, Shuffle, PenLine, ChevronRight
} from 'lucide-react';
import { 
...
const SystemManagerDashboard: React.FC<Props> = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState<'USERS' | 'GATES' | 'SYSTEM' | 'LOGS'>('USERS');
  ...
  const handleSaveUser = async () => {
    ...
    setLoading(true);
    const payload = { ...userForm, pin_code: tempPassword };
  ...
  const handleSaveGate = async () => {
      ...
      setLoading(true);
      const success = await saveGateConfig(gateForm);
      setLoading(false);
      if(success) {
