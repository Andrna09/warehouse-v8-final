...
import { Clock, MapPin, RefreshCw, Truck, FileText, CheckCircle, ArrowLeft, Loader2, Megaphone } from 'lucide-react';
...
const DriverStatus: React.FC<Props> = ({ driverId, onBack }) => {
  ...
  const fetchStatus = async () => {
    ...
    if (data) {
      ...
      if (data.status === QueueStatus.VERIFIED) {
        ...
        // Count Verified drivers checked in before me
  ...
  useEffect(() => {
    fetchStatus();
    const interval = setInterval(fetchStatus, 10000); // Poll slower for public
    ...
    switch (status) {
      ...
      case QueueStatus.VERIFIED: 
        return {
           ...
           title: 'Dalam Antrian',
           desc: 'Silakan parkir dan tunggu panggilan via WhatsApp/Layar.',
      ...
      case QueueStatus.CALLED: 
        return {
           ...
           bg: 'bg-indigo-50 border-indigo-200 animate-pulse'
        };
  ...
  return (
    <div className="max-w-xl mx-auto animate-fade-in-up pb-10 px-4 pt-28 md:pt-32">
